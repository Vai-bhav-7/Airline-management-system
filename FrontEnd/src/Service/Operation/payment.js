...
import { setAllBookingDetails } from "../../Slices/flightSlice";
...
export function createPayment(bookingDetail, navigate, userData) {
  return async (dispatch, getState) => {
    const toastId = toast.loading("Processing Payment...");

    try {
      // 1. Backend ko fake payment bhej rahe hain
      const result = await apiConnector("POST", CREATE_PAYMENT, bookingDetail);
      console.log("Fake Payment Success:", result.data);

      // 2. Payment successful toast
      toast.dismiss(toastId);
      toast.success("Payment Successful! 🎉");

      // 3. Fake boarding pass banate hain
      const bookingWithPass = {
        ...bookingDetail,
        departureTime: bookingDetail.departureTime,
        arrivalTime: bookingDetail.arrivalTime,
      ...
      // 4. Redux me store karo
      const currentBookings = getState().flight.allbookingDetails || [];
      dispatch(setAllBookingDetails([...currentBookings, bookingWithPass]));

      // 5. Local storage me bhi store kar lo (slice use kar raha hai)
      localStorage.setItem(
        "allbookingDetails",
        JSON.stringify([...currentBookings, bookingWithPass])
      );

      // 6. Navigate to MyBooking / Boarding Pass page
