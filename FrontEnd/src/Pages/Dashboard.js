...
const Dashboard = () => {
  ...
  async function submitHandler(event) {
    ...
    if (!formData.flightFrom || !formData.flightTo || !formData.date) {
    ...
    // Create a new object with day included (do NOT mutate state directly)
    const updatedFormData = { ...formData, day: dayOfWeek };
  
    console.log("Submitting:", updatedFormData);
  
    // Dispatch the action with updated data
    await dispatch(searchFlightData(updatedFormData, navigate));
  }
  
  // Generic handler to update formData state
